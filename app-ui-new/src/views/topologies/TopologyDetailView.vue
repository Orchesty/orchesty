...
const processesTabRef = ref<InstanceType<typeof TopologyProcessesTab> | null>(null)
const logsTabRef = ref<InstanceType<typeof TopologyLogsTab> | null>(null)
const failedMessagesTabRef = ref<InstanceType<typeof TopologyFailedMessagesTab> | null>(null)
...
const handleTabChange = (tabId: string) => {
  ...
  if (tabId === 'processes') {
    processesTabRef.value?.loadData()
  } else if (tabId === 'logs') {
    logsTabRef.value?.loadData()
  } else if (tabId === 'trash') {
    failedMessagesTabRef.value?.loadData()
  }
  if (topology.value) {
    setLastTopology({
      id: props.id,
  ...
  <div v-else-if="topology" class="px-4 pt-2 pb-4">
    ...
    >
      ...
      <div id="processes-content" role="tabpanel" aria-labelledby="processes-tab" class="hidden">
        <TopologyProcessesTab ref="processesTabRef" v-if="topology" :topology-id="topology._id" :topology-name="topology.name" />
      </div>

      <div id="logs-content" role="tabpanel" aria-labelledby="logs-tab" class="hidden">
        <TopologyLogsTab ref="logsTabRef" v-if="topology" :topology-id="topology._id" :topology-name="topology.name" />
      </div>

      <div id="trash-content" role="tabpanel" aria-labelledby="trash-tab" class="hidden">
        <TopologyFailedMessagesTab ref="failedMessagesTabRef" v-if="topology" :topology-id="topology._id" :topology-name="topology.name" />
      </div>
