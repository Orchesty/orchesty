...
Integrace s Orchesty znamená v podstatě připojení integrované služby k orchestrační vrstvě. Zatím co orchestrační vrstva řeší odkud kam a za jakých podmínek směřují data, integrační vrstva řeší komunikaci mezi službami a orchestrační vrstvou a umožňuje budování komponent integračních procesů, jako jsou konektory, transformace dat nebo filtry.
...
V takové službě můžeme vytvářet libovolné akce, které jsou pak dostupné v editoru procesů. Pomocí instalovaného SDK můžeme orchestrovat v podstatě libovolné služby nebo mikroslužby.
...
we can use prebuilt connectors from [Orchesty Store](../orchesty/app-store), or build custom connectors when necessary.
