...
:::note
Some of these settings are handled in parallel by means other than message headers. However, they have their reasons here and the control service prefers specific definitions in specific situations.
:::

## Data
Data is transmitted in Orchesty messages in the form of a string. It is up to us what data format we choose and how we validate the data. However, to embed a data object in JSON format, **ProcessDto** provides the `setJsonData` or `setNewJsonData` method.
