...
export const NAME = 'forgot-password-email-mapper';
...
export default class ForgotPasswordEmailMapper extends ASystemEmailMapper {
    ...
    public async processAction(dto: ProcessDto<IPayload>): Promise<ProcessDto<IInput>> {
        ...
        const { email, hash, frontendUrl } = dto.getJsonData();
        const baseUrl = frontendUrl.replace(/\/+$/, '');
        const resetUrl = `${baseUrl}/reset-password/${hash}`;
        ...
        <tr>
          <td style="padding:40px">
            ...
            <p style="margin:0 0 24px;color:#4b5563;font-size:15px;line-height:1.6">
              We received a request to reset the password for your Orchesty account (<strong>${email}</strong>). Click the button below to set a new password.
            </p>
            <table cellpadding="0" cellspacing="0" style="margin:0 0 24px">
              <tr>
                <td style="background-color:#1bea83;border-radius:6px;padding:12px 32px">
                  <a href="${resetUrl}" style="color:#000000;text-decoration:none;font-size:15px;font-weight:600;display:inline-block">Reset Password</a>
                </td>
              </tr>
            </table>
            <p style="margin:0 0 8px;color:#6b7280;font-size:13px">Or copy and paste this link into your browser:</p>
            <p style="margin:0 0 24px;color:#1bea83;font-size:13px;word-break:break-all">${resetUrl}</p>
