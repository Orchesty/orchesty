...
export const NAME = 'ecomail-send-sales-business-notification';

function buildSubject(ctx: ISalesFormContext): string {
    const fullName = `${ctx.firstName} ${ctx.lastName}`;
    const companySuffix = ctx.company ? ` (${ctx.company})` : '';
    if (ctx.locale?.toLowerCase().startsWith('cs')) {
        return `Nový sales lead: ${fullName}${companySuffix}`;
    }
    return `New sales lead: ${fullName}${companySuffix}`;
}

export default class EcomailSendSalesBusinessNotificationConnector extends EcomailSendTransactionalEmailConnector {
    ...
    public async processAction(dto: ProcessDto<IInput>): Promise<ProcessDto<IOutput>> {
        ...
        checkParams(
            ...
            ['firstName', 'lastName', 'personId', 'leadId'],
        );
        ...
        const businessEmail = settings?.[BUSINESS_EMAIL] as string | undefined;
        const templateId = settings?.[SALES_BUSINESS_TEMPLATE_ID] as number | string | undefined;

        const missing: string[] = [];
        if (!fromEmail) missing.push(FROM_EMAIL);
        if (!fromName) missing.push(FROM_NAME);
        if (!businessEmail) missing.push(BUSINESS_EMAIL);
        if (templateId === undefined || templateId === null || templateId === '') missing.push(SALES_BUSINESS_TEMPLATE_ID);
        ...
        return ctxDto.setNewJsonData<ISalesFormContext>({
            ...ctx,
            businessEmailId: emailOutput.id,
