...
func (h *Swarm) RunAction(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	var (
	...
	if len(topologyId) != 0 {
		...
		if topology.ID.Hex() != "" {
			...
			topologyJson := fmt.Sprintf("%s/%s/topology.json", viper.GetString("generator.path"), topology.GetSaveDir())

			var err error

			_, _, _ = commands.Execute(
				"docker",
				"config",
				"create",
				docker_compose.GetConfigName(topology),
				topologyJson,
			)
			_, err, stdErr = commands.Execute(
				...
				"-c",
				config,
				docker_compose.GetTopologyPrefix(topology),
			)
	...
	log.Printf("Swarm RunAction: %s", message)
