...
export default {
  ...
  methods: {
    ...
    async saveTopology(form, xml) {
      ...
      await this[TOPOLOGIES.ACTIONS.TOPOLOGY.SAVE_DIAGRAM]({ id: response._id, xml: xml })
    ...
    async replaceTopologyData(e, altName = '', folderId, setDiagramState) {
      ...
      reader.onload = async (event) => {
        ...
        if (setDiagramState) {
          await this[TOPOLOGIES.ACTIONS.TOPOLOGY.SET_LOCAL_DIAGRAM](new XMLSerializer().serializeToString(xmlDoc))
          this.isOpen = false
        } else {
    ...
    fetchTopologyDiagram(e, folderId, setDiagramState = false) {
      ...
      reader.onload = (event) => {
        ...
        events.emit(EVENTS.MODAL.TOPOLOGY.IMPORT, { e, folderId, setDiagramState })
      }
      reader.readAsText(file)
    },
  },
