  ...
  <div>
    ...
    <v-row v-if="!state.isSending" key="2" dense class="d-flex justify-start">
      <v-col cols="12">
        <v-card outlined class="bpmn-viewer-node-grid-container">
          ...
          <div
            v-if="Object.keys(selectedNode).length"
            class="bpmn-viewer-node-grid"
          >
            <bpmn-node-grid
              :node="selectedNode"
              :filter="filter"
    ...
    <v-row v-else key="1">
      <v-col cols="12">
        ...
        >
          <progress-bar-linear />
          <h4 class="font-weight-medium mt-5">
            {{ $t("page.status.loading") }}
          </h4>
        </v-card>
      </v-col>
    </v-row>
...
export default {
  ...
  methods: {
    ...
    hasTests(topology) {
      ...
      this.testResultsShow = topology.test
        ? Object.keys(topology.test).length === 0
        : true
    },
    ...
    setStartingPointsColor() {
      this.topologyActiveNodes.forEach((node) => {
        if (
          ...
          svg.forEach((svg) => {
            !node.enabled
              ? (svg.style.fill = "#D11818")
              : (svg.style.fill = "#66C600")
          })
        }
      })
    },
