...
export default {
  ...
  methods: {
    ...
    async submit() {
      await this[TOPOLOGIES.ACTIONS.TOPOLOGY.DELETE]({ id: this.topology._id, name: this.topology.name }).then(
        async (res) => {
          if (res) {
    ...
    async disable() {
      await this[TOPOLOGIES.ACTIONS.TOPOLOGY.DISABLE](this.topology.id).then((res) => {
        if (res) {
          this.isOpen = false
        }
      })
