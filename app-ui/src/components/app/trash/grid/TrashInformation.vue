  ...
  <div v-else>
    <v-col cols="12" class="d-flex flex-column">
      <v-row class="flex-grow-0 flex-shrink-1">
        <v-col cols="12" md="3">
          <h3>Node</h3>
          <div class="text-uppercase">{{ $t('topologies.userTask.information.nodeName') }}</div>
          <div class="truncate_header">{{ item ? item.id : '' }}</div>
        </v-col>
        <v-col cols="12" md="9">
          <div class="d-flex text-center justify-space-around height-100">
            <div class="flex-item">
              <h5>{{ $t('topologies.userTask.information.created') }}</h5>
              <span>{{ item ? $options.filters.toLocalDate(item.created) : '' }}</span>
            </div>
            <div class="flex-item">
              <h5>{{ $t('topologies.userTask.information.parentID') }}</h5>
              <span class="px-1">{{ item ? item.processId : '' }}</span>
            </div>
            <div class="flex-item">
