<template>
  ...
  >
    <template #button>
      <app-button :on-click="() => (isOpen = !isOpen)" icon>
        <template #icon>
...
export default {
  ...
  components: { AppIcon, AppButton, ModalTemplate, EmailSender, AmqpForm, CurlForm },
  data() {
    return {
      isOpen: false,
      NOTIFICATION_TYPES,
  ...
  methods: {
    ...
    async onSubmit(values) {
      const response = await this[NOTIFICATIONS.ACTIONS.UPDATE_NOTIFICATIONS_REQUEST]({
        id: this.service.id,
