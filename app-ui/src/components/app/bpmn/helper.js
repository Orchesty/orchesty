export function fitIntoScreen(viewer, diagram, margins = [20, 60]) {
  ...
  for (let i = 0; i < xml.getElementById('BPMNDiagram_1')?.firstElementChild?.children.length; i++) {
  ...
  if (left !== undefined) {
    ...
    const scale = Math.min(
      scaleLimit[1],
      Math.max(Math.min(maxW / (right - left), maxH / (bottom - top)), scaleLimit[0])
    )
    maxW = maxW / scale - margins[0]
    maxH = maxH / scale - margins[1]

    canvas.viewbox({
      x: maxW > right - left ? left - (maxW - right + left) * 0.5 : left - margins[0],
      y: maxH > bottom - top ? top - (maxH - bottom + top) * 0.5 : top - margins[1],
      width: maxW,
      height: maxH,
    })
  }
