...
import AlertLogs from '@/components/app/dashboard/grid/AlertLogs'

export default {
  name: 'TopologyDashboard',
  components: { AlertLogs, ErrorLogs, Doughnut },
  methods: {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: this.$t('topologies.dashboard.headers.topologyName'), value: 'name', align: 'center', width: '20%' },
        { text: this.$t('topologies.dashboard.headers.message'), value: 'message', align: 'center', width: '45%' },
        { text: this.$t('topologies.dashboard.headers.level'), value: 'level', align: 'center', width: '20%' },
      ],
      stats: {
  ...
  computed: {
    ...mapState(TOPOLOGIES.NAMESPACE, ['dashboard']),
    ...mapGetters(REQUESTS_STATE.NAMESPACE, [REQUESTS_STATE.GETTERS.GET_STATE]),
    state() {
      return this[REQUESTS_STATE.GETTERS.GET_STATE]([API.topology.getDashboard.id])
    },
    alertLogs() {
      return this.dashboard?.alertLogs
    },
    errorLogs() {
      return this.dashboard?.errorLogs
    },
    process() {
      return this.dashboard?.process
    },
    systemMetrics() {
      return this.dashboard?.systemMetrics
    },
    filter() {
