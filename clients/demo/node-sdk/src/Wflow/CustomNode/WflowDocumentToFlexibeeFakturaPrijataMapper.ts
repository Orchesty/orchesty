...
import { FakturaPayload, Payload, PolozkaFaktury, ZavazekPayload } from '../../FlexiBee/types/payload';
...
export default class WflowDocumentToFlexibeeFakturaPrijataMapper extends ACommonNode {
    ...
    public static parseAddress(address: string | null | undefined): { ulice: string; mesto: string; psc: string } {
    ...
    public processAction(dto: ProcessDto<IInput>): ProcessDto<IOutput> {
        ...
        const firma = `code:${dto.getHeader(FIRMA_KOD)}` as const;
...
export type IOutput = Payload<FakturaPayload | ZavazekPayload>;
