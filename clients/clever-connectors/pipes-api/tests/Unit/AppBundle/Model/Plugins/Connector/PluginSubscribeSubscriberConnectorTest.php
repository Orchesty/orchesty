...
use CleverConnectors\AppBundle\Model\Plugins\Connector\PluginSubscribeSubscriberConnector;
...
/**
 * Class PluginSubscribeSubscriberConnectorTest
 *
 * @package Tests\Unit\AppBundle\Model\Plugins\Connector
 */
final class PluginSubscribeSubscriberConnectorTest extends KernelTestCaseAbstract
{
            ...
            ->method('send')->will($this->returnCallback(
                function (RequestDto $requestDto) {
                    $dto = new RequestDto(CurlManager::METHOD_POST,
                        new Uri('https://neco.com/clever_connector/subscriber/subscribe?id=someId'));
                    $dto
                        ->setBody('{}')
                        ->setHeaders([]);
