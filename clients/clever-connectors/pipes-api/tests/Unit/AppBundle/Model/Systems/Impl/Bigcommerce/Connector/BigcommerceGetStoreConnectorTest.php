...
use CleverConnectors\AppBundle\Model\Systems\Impl\Bigcommerce\Connector\BigcommerceGetStoreConnector;
...
final class BigcommerceGetStoreConnectorTest extends ConnectorTestCaseAbstract
{
    ...
    public function testProcessAction(): void
    {
        $result = Json::decode(
            $this->getConnectorMock(function (RequestDto $dto, array $options = []) {
                $this->assertEquals(
                    new Uri('https://api.bigcommerce.com/stores/ukcfcghi/v2/store'),
                    $dto->getUri()
                );

                return new ResponseDto(200, 'OK', $this->getRequest('BigcommerceStoreResponse.json'), []);
            })->processAction((new ProcessDto())->setHeaders([]))->getData(),
            TRUE
        );
    ...
    public function testProcessActionWithLimiter(): void
    {
        ...
        })->processAction((new ProcessDto())->setHeaders([]))->getHeaders();
    ...
    ): BigcommerceGetStoreConnector
    {
        ...
        $connector = new BigcommerceGetStoreConnector($system, $documentManager, $curlManager);
