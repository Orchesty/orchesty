...
use CleverConnectors\AppBundle\Model\Systems\Impl\Pipedrive\Mapper\PipedriveCreatedPersonMapper;
...
final class PipedriveCreatedPersonMapperTest extends ConnectorTestCaseAbstract
{
     ...
     *
     */
    public function testProcessEventCreated(): void
    {
        ...
        $connector = new PipedriveCreatedPersonMapper($dm);

        $response = Json::decode(
            $connector->process((new ProcessDto())->setData($this->getRequest('personCreateWebhook.json'))
                ->setHeaders([]))
                ->getData(),
            TRUE
        );
