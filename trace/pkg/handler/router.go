...
func writeErrorResponse(writer http.ResponseWriter, err error, rawBody ...[]byte) {
	...
	if len(rawBody) > 0 {
		_, _ = writer.Write(rawBody[0])
	} else if err := json.NewEncoder(writer).Encode(map[string]interface{}{"message": message}); err != nil {
		logContext().Error(err)
	}
}
