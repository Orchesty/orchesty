<template>
  ...
  >
    <template #default="{ items }">
      ...
      <td v-for="(notification, key) in notificationItems" :key="key" class="text-center">
        <tooltip v-if="updating.isSending || notification.settings.length === 0">
          ...
          <template #tooltip>
            <span>{{ updating.isSending ? 'Service is being updated' : 'Set the service first' }}</span>
          </template>
        </tooltip>
        <v-switch
          v-else
...
export default {
  ...
  computed: {
    ...
    ...mapState(NOTIFICATIONS.NAMESPACE, ['notifications', 'events']),
