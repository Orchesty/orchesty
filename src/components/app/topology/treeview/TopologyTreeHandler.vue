...
export default {
  ...
  methods: {
    ...
    async onActive(values) {
      if (!values.length) {
        await this.$router.push({ name: ROUTES.TOPOLOGIES.DEFAULT })
        return
      }
      if (values[0].type === TOPOLOGY_TREE.FOLDER) return
      if (this.$route.params.id === values[0].id) return
      await this.$router.push({ name: ROUTES.TOPOLOGIES.EDITOR, params: { id: values[0].id } })
    },
