...
const {
  createPaginationList, createCompleteList, listLoading, listError, listReceive, listDelete, listChangeSort, listChangePage, listChangeFilter, invalidateLists,
} = listFactory('TOPOLOGY/LIST/');
...
export function needTopology(id, force = false) {
  return (dispatch, getState) => {
    ...
    if (!topology || force) {
      ...
      return serverRequest(dispatch, 'GET', `/topologies/${id}`).then((response) => {
        if (response) {
...
export function topologyUpdate(id, data) {
  return (dispatch, getState) => {
    ...
    return serverRequest(dispatch, 'PATCH', `/topologies/${id}`, null, data).then((response) => {
      if (response) {
        const oldTopology = getState().topology.elements[id];
        dispatch(receive(response));
        dispatch(topologyGroupActions.recalculateTopologyGroup(response.name, oldTopology));
...
export function topologyCreate(data, processHash = 'new') {
  return (dispatch) => {
    ...
    return serverRequest(dispatch, 'POST', '/topologies', null, data).then((response) => {
      if (response) {
        dispatch(receive(response));
...
export function topologyDelete(id, redirectToList = false) {
  return (dispatch) => {
    ...
    return serverRequest(dispatch, 'DELETE', `/topologies/${id}`).then((response) => {
      if (response) {
        dispatch(invalidateLists());
        if (redirectToList) {
...
export function cloneTopology(id, silent = false) {
  return (dispatch) => {
    ...
    return serverRequest(dispatch, 'POST', `/topologies/${id}/clone`).then((response) => {
      if (response) {
        if (!silent) {
...
export function publishTopology(id, silent = false) {
  return (dispatch, getState) => {
    ...
    return serverRequest(dispatch, 'POST', `/topologies/${id}/publish`).then((response) => {
      if (response) {
        if (!silent) {
