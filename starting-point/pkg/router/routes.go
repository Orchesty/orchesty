...
func GetDefaultRoutes() Routes {
	return Routes{
		...
		Route{
			...
			Method:      "POST",
			Pattern:     "/human-tasks/topologies/{topology}/nodes/{node}/run",
			HandlerFunc: HandleClear(HandleHumanTaskRunByID),
		},
		Route{
			Name:        "Run human task topology by ID with token",
			Method:      "POST",
			Pattern:     "/human-tasks/topologies/{topology}/nodes/{node}/token/{token}/run",
			HandlerFunc: HandleClear(HandleHumanTaskRunByID),
		},
		Route{
			Name:        "Run human task topology by name",
			Method:      "POST",
			Pattern:     "/human-tasks/topologies/{topology}/nodes/{node}/run-by-name",
			HandlerFunc: HandleClear(HandleHumanTaskRunByName),
		},
		Route{
			Name:        "Run human task topology by name with token",
			Method:      "POST",
			Pattern:     "/human-tasks/topologies/{topology}/nodes/{node}/token/{token}/run-by-name",
			HandlerFunc: HandleClear(HandleHumanTaskRunByName),
		},
		Route{
			Name:        "Stop human task topology by ID",
			Method:      "POST",
			Pattern:     "/human-tasks/topologies/{topology}/nodes/{node}/stop",
			HandlerFunc: HandleClear(HandleHumanTaskStopByID),
		},
		Route{
			Name:        "Stop human task topology by ID with token",
			Method:      "POST",
			Pattern:     "/human-tasks/topologies/{topology}/nodes/{node}/token/{token}/stop",
			HandlerFunc: HandleClear(HandleHumanTaskStopByID),
		},
		Route{
			Name:        "Stop human task topology by name",
			Method:      "POST",
			Pattern:     "/human-tasks/topologies/{topology}/nodes/{node}/stop-by-name",
			HandlerFunc: HandleClear(HandleHumanTaskStopByName),
		},
		Route{
			Name:        "Stop human task topology by name with token",
			Method:      "POST",
			Pattern:     "/human-tasks/topologies/{topology}/nodes/{node}/token/{token}/stop-by-name",
			HandlerFunc: HandleClear(HandleHumanTaskStopByName),
		},
		Route{
