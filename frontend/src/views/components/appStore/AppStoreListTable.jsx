    ...
    return list && list.items ? list.items.map(id => {
      ...
      return (
        <tr key={key} className={authorized !== undefined ? (authorized ? 'text-success' : 'text-danger') : null}
          ...
          <td
            className="col-md-2">{applicationType.substr(0, 1).toUpperCase() + applicationType.substr(1).toLowerCase()}</td>
          <td
            className="col-md-2">{authorizationType.substr(0, 1).toUpperCase() + authorizationType.substr(1).toLowerCase()}</td>
