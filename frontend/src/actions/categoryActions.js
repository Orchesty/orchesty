...
const {
  createCompleteList, listLoading, listReceive, listError, invalidateLists,
} = listFactory('CATEGORY/LIST/');
...
export function createCategory(data, processHash = 'new') {
  return (dispatch) => {
    ...
    return serverRequest(dispatch, 'POST', '/categories', null, data).then((response) => {
      if (response) {
...
export function updateCategory(id, data) {
  return (dispatch) => {
    ...
    return serverRequest(dispatch, 'PATCH', `/categories/${id}`, null, data).then((response) => {
      if (response) {
        dispatch(receive(response));
      }
...
export function deleteCategory(id) {
  return (dispatch) => {
    ...
    return serverRequest(dispatch, 'DELETE', `/categories/${id}`).then((response) => {
      if (response) {
        dispatch(invalidateLists());
        dispatch(invalidateTrees());
        dispatch(remove(id));
